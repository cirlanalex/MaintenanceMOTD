...
public class Messages {
    ...
    public void loadConfiguration() {
        try {
            messagesConfig = ConfigurationProvider.getProvider(YamlConfiguration.class).load(new File(MaintenanceMOTD.getInstance().getDataFolder(), "messages.yml"));
        } catch (IOException e) {
            MaintenanceMOTD.getInstance().getLogger().warning("§c[§4MaintenanceMOTD§c] §cAn error occurred while trying to load the messages.yml file");
            // e.printStackTrace();
        }
    }
